...
- `prove-it`: Build evidence-backed proof matrices for debugging, validation, comparisons, and other work where the claim needs ground truth. USE WHEN: Debugging when the cause is uncertain and several hypotheses could explain the symptom. | Validating non-trivial behavior across roles, states, platforms, runtimes, or side effects. | Comparing candidates where performance, reliability, cost, or quality is the claim. | Defining the right ground truth before changing the system. | Use when the user says or implies: | "Prove it." | "Define ground truth." | "Make a matrix." | "Loop until the matrix matches." | "Don't speculate." | "Compare these options." | "Validate this live."
