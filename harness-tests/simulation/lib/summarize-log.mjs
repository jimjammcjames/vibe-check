...
for (let i = 0; i < lines.length; i++) {
  ...
  if (
    line.includes("exec npm run harness:") ||
    line.includes("> harness-sandbox@1.0.0 harness:") ||
    line.includes("> acme-core@1.0.0 harness:")
  ) {
    const cmdMatch =
      line.match(/exec (npm run harness:[\w:-]+)/) ||
      line.match(/> (?:harness-sandbox|acme-core)@1\.0\.0 (harness:[\w:-]+)/);
    if (cmdMatch) {
      if (capturingOutput) {
        analyzeAndPrint(lastHarnessCommand, outputBuffer);
      }

      const cmd = cmdMatch[1].startsWith("harness:")
        ? `npm run ${cmdMatch[1]}`
        : cmdMatch[1];
  ...
  if (
    capturingOutput &&
    line.match(/npm run harness:[\w:-]+ (succeeded|failed|exited)/)
  ) {
  ...
  if (
    capturingOutput &&
    (line.includes("[simulation] Full log saved to:") ||
      line.includes("[simulation] Cleaning up sandbox") ||
  ...
  if (findingReaction) {
    if (line.includes("thinking") || line.includes("codex")) {
      ...
      if (reaction.length > 0) {
        const filteredReaction = reaction.filter(
          (l) => !l.startsWith("exec ") && !l.startsWith("tokens used:"),
        );
  ...
  if (failed) {
    ...
    if (!foundSpecificError) {
      ...
      const errLines = lines.filter(
        (l) => l.includes("fail") || l.includes("Error"),
      );
