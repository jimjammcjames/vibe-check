...
function analyzeTestContent(content) {
    ...
    if (hasNetworkUsage && !hasNetworkGate) {
        issues.push({
            ...
            message: 'Network usage in tests must be gated by HARNESS_ALLOW_NETWORK_TESTS'
        });
    }

    const keyFileUsage = content.includes('key.txt') &&
    ...
    if (readsSource) {
        issues.push({
            ...
            message: 'Tests should not inspect source files via readFileSync; assert behavior instead'
        });
    }
