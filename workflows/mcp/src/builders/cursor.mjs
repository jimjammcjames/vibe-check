/**
 * Cursor (.cursor/mcp.json) builder.
 *
 ...
 */
export function buildCursorServerEntry(server, namespace, envMap) {
...
/**
 * Build stdio entry for Cursor with resolved literals.
 * @param {object} server
...
/**
 * Build http entry for Cursor with resolved literals.
 * @param {object} server
 * @param {Record<string, string>} envMap
 * @returns {object}
 */
function buildHttpEntry(server, envMap) {
  ...
  // Build headers with resolved values
  const headers = { ...(server.headers ?? {}) };

  // If bearer auth, resolve the token
